<script lang="ts">
	...
	let pickupImages = [] as Array<{ id: string; name: string; data: string }>;
	...
	async function handlePickup() {
		...
		try {
			error = null;
			const response = await pickUpProduct(rentContractId, {
				pickupImages: pickupImages.map((image) => image.data),
			});
			...
			error =
				err?.errorMessages?.[0]?.message || "Failed to confirm pickup";
		} finally {
	...
	async function cancelContract() {
		if (!rentContract) return;
		await cancelRentContract(rentContract.id);
		goto(ROUTES.CUSTOMER_PROFILE);
	}
	...
	function removeImage(id: string) {
		pickupImages = pickupImages.filter((image) => image.id !== id);
	}
</script>
		...
		<div class="col-span-7">
			{#if rentContract && product}
				<div class="rounded-md border p-4 mb-4">
					<p class="text-xl font-bold">{$t("shop.pickup.title")}</p>
					<p class="text-sm text-gray-500 mt-2">
						{$t("shop.pickup.description")}
					</p>
				</div>

				<div class="rounded-md border p-6 my-4">
					<p class="text-xl font-bold">
						{$t("shop.pickup.damage.title")}
					</p>
					<p class="text-sm text-gray-500 mt-2">
						{$t("shop.pickup.damage.description")}
					</p>
					...
					<div class="flex flex-col gap-2">
						<p class="text-sm font-bold">
							{$t("shop.pickup.damage.description.title")}
						</p>
						<Textarea.Root
							bind:value={damageDescription}
							disabled={!damage}
							placeholder={$t(
								"shop.pickup.damage.description.placeholder",
							)}
						/>
					</div>
				</div>
				...
				<div class="rounded-md border p-6 my-4">
					<p class="text-xl font-bold">
						{$t("shop.pickup.images.title")}
					</p>
					<p class="text-sm text-gray-500 mt-2">
						{$t("shop.pickup.images.description")}
					</p>

<script lang="ts">
	...
	import { fetchRentContractById, pickUpProduct } from "$lib/states/rent_contracts/effects";
	...
	async function handlePickup() {
		...
		try {
			...
			if (!response?.errorMessages) {
				...
				// Error with messages
				error = response.errorMessages[0].message || "Failed to confirm pickup";
			}
		} catch (err: any) {
			console.error("Pickup confirmation failed:", err);
			error = err?.errorMessages?.[0]?.message || "Failed to confirm pickup";
		}
	}
</script>
...
<div class="container mx-auto p-4">
	...
	{:else if rentContract}
		...
		<form on:submit|preventDefault={handlePickup} class="pickup-form">
			<Button variant="default" type="submit">Confirm Pickup</Button>
		</form>
	{:else}

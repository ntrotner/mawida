...
func (s *ProductAPIService) ProductsProductIdRentPost(ctx context.Context, productId string, rentProductFormular models.RentProductFormular, r *http.Request) (models.ImplResponse, error) {
	...
	startDate := time.Unix(rentProductFormular.RentalStartDate, 0).In(loc)
	endDate := time.Unix(rentProductFormular.RentalEndDate, 0).In(loc)
	...
	todayAtZero := time.Date(today.Year(), today.Month(), today.Day(), 0, 0, 0, 0, loc)
	tomorrow := todayAtZero.AddDate(0, 0, 1)
	tomorrowAtZero := time.Date(tomorrow.Year(), tomorrow.Month(), tomorrow.Day(), 0, 0, 0, 0, loc)
	if !(startDate.After(todayAtZero) || startDate.Equal(todayAtZero)) && startDate.Before(tomorrowAtZero) {
